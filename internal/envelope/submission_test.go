...
func TestSubmissionBindsActorPRHeadAndBundle(t *testing.T) {
	...
	verified, err := VerifySubmission(raw, Expected{EventID: "summer-data-2026", EventEpoch: "1", RepositoryID: "100", ActorID: "42", ConfigDigest: strings.Repeat("4", 64), Now: issued.Add(time.Minute)}, registry)
	if err != nil {
		t.Fatal(err)
	}
	...
	if _, err := VerifySubmission(mutated, Expected{}, registry); err == nil {
